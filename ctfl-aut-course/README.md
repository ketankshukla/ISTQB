# CTFL-AuT — Certified Tester Foundation Level Automotive Software Testing

Complete self-study course for the ISTQB CTFL-AuT certification exam.

---

## Exam Facts

| Parameter | Value |
|-----------|-------|
| Certification | ISTQB CTFL-AuT — Foundation Level Automotive Software Testing |
| Syllabus version | v1.0 |
| Questions | 20 |
| Duration | 40 minutes |
| Pass mark | 65% (13/20) |
| Format | Closed book, single best answer |
| K-levels tested | K1 (remember), K2 (understand), K3 (apply) |

---

## Course Structure

| Chapter | Topic | Est. Weight |
|---------|-------|-------------|
| 1 | Introduction to Automotive Testing | ~15% |
| 2 | Automotive Software Basics | ~20% |
| 3 | Automotive Testing Process | ~20% |
| 4 | Automotive Testing Techniques | ~15% |
| 5 | Automotive Test Environment | ~15% |
| 6 | Automotive Quality and Standards | ~15% |

---

## How to Use This Course

1. **Read `STUDY_PLAN.md`** for a realistic 2-week schedule
2. **Work through chapters in order** — read lessons, study exam traps, attempt practice questions
3. **Complete case studies** to integrate techniques end-to-end
4. **Take mock exams** under timed conditions (closed book)
5. **The day before your exam**, review `EXAM_STRATEGY.md` and the glossary

---

## Target Audience

- Testers with CTFL who want to understand automotive testing fundamentals
- Automotive testing newcomers
- Anyone preparing for the ISTQB CTFL-AuT exam

---

## Question Bank Summary

| Source | Questions |
|--------|-----------|
| Chapter practice questions | 120 |
| Mock exams (3 x 20) | 60 |
| **Total** | **180** |

---

## Prerequisites

- ISTQB Foundation Level (CTFL) certification or equivalent knowledge

---

## Disclaimer

This is an independent study resource. It is not affiliated with, endorsed by, or produced by ISTQB or any ISTQB member board. ISTQB and Certified Tester are registered trademarks. The official syllabus and sample exams are available from [istqb.org](https://www.istqb.org).
