# 📝 CTFL-AT Mock Exam 1

**Instructions:**
- 40 questions
- 60 minutes
- Single best answer for each question
- No negative marking

---

### 🔷 Q1

According to the Agile Manifesto, which is valued MORE?

A. Processes and tools over individuals and interactions
B. Comprehensive documentation over working software
C. Customer collaboration over contract negotiation
D. Following a plan over responding to change

---

### 🔷 Q2

In an agile team using the whole-team approach to quality:

A. Testers are no longer needed because everyone tests
B. Quality is the responsibility of the entire team, with testers providing specialized expertise
C. Testers should become developers to contribute more effectively
D. Testing is performed only by a separate independent test team

---

### 🔷 Q3

Which Agile Manifesto principle directly supports the practice of welcoming changing requirements?

A. Deliver working software frequently
B. Welcome changing requirements, even late in development
C. Business people and developers work together daily
D. Build projects around motivated individuals

---

### 🔷 Q4

In Scrum, system testing is performed:

A. In a dedicated phase after all stories are developed
B. Continuously throughout the sprint as each story is completed
C. Only during the sprint review meeting
D. Once per release, before deployment

---

### 🔷 Q5

The test automation pyramid shows that the LARGEST proportion of automated tests should be:

A. UI tests
B. Integration tests
C. Unit tests
D. Manual tests

---

### 🔷 Q6

In ATDD, acceptance tests are defined:

A. After the code is written
B. Before development begins, collaboratively by the team
C. Only by the tester, independently
D. During the final sprint review

---

### 🔷 Q7

Which format is commonly used in BDD to express acceptance criteria?

A. If-Then-Else
B. Given-When-Then
C. Input-Process-Output
D. For-Each-Do

---

### 🔷 Q8

A tester participates in a Three Amigos meeting. Their PRIMARY contribution is to:

A. Write the production code for the story
B. Identify edge cases, risks, and testability issues
C. Create the project schedule
D. Approve the budget

---

### 🔷 Q9

Continuous Integration is BEST described as:

A. Automatically deploying code to production on every commit
B. Frequently merging code changes with automated builds and tests
C. Writing tests after the code is developed
D. Manual testing performed continuously

---

### 🔷 Q10

Exploratory testing in agile is:

A. Random clicking without any plan
B. Structured by test charters and time-boxed sessions
C. Only performed at the end of the sprint
D. A replacement for all automated testing

---

### 🔷 Q11

A team has 400 UI tests, 50 integration tests, and 30 unit tests. This distribution is:

A. An ideal test automation pyramid
B. An ice cream cone anti-pattern
C. A balanced approach
D. Sufficient for all agile testing needs

---

### 🔷 Q12

The PRIMARY purpose of Specification by Example is to:

A. Generate test scripts automatically
B. Create shared understanding of requirements using concrete examples
C. Replace all manual testing
D. Eliminate the need for a product owner

---

### 🔷 Q13

In agile, the Definition of Done typically includes:

A. Only that the developer has written the code
B. That the feature is coded, tested, reviewed, and potentially releasable
C. That comprehensive documentation has been created
D. That an external test team has approved the feature

---

### 🔷 Q14

TDD, ATDD, and BDD are related practices. Which statement is TRUE?

A. They are all the same practice with different names
B. TDD is unit-level; ATDD and BDD are acceptance-level and team-focused
C. BDD replaces TDD and ATDD completely
D. ATDD is only for manual testing

---

### 🔷 Q15

A team's CI build fails randomly due to tests that pass on rerun. These tests are:

A. Regression tests
B. Flaky tests
C. Unit tests
D. Acceptance tests

---

### 🔷 Q16

Which of the following is a benefit of early tester involvement in story refinement?

A. Testers can write the production code
B. Defects are prevented by clarifying requirements before coding
C. Testers can avoid talking to developers
D. The team needs fewer user stories

---

### 🔷 Q17

In the test automation pyramid, UI tests are characterized by:

A. Being fast and easy to maintain
B. Being slow, brittle, and expensive to maintain
C. Requiring no programming skills
D. Being the most reliable type of test

---

### 🔷 Q18

A team wants fast feedback on code changes. Which tests should run FIRST in CI?

A. Full end-to-end UI regression suite
B. Unit test suite
C. Manual exploratory testing session
D. Performance test suite

---

### 🔷 Q19

Which practice involves business, developer, and tester meeting to discuss a story before implementation?

A. Sprint retrospective
B. Three Amigos
C. Bug triage meeting
D. Release planning

---

### 🔷 Q20

A team delays implementing test automation until after 5 sprints. What is the MAIN risk?

A. The team will have too much time for manual testing
B. The codebase becomes harder to test, and manual regression becomes unsustainable
C. Agile does not require test automation
D. Test automation is only useful for waterfall projects

---

### 🔷 Q21

In agile, regression testing is typically:

A. Performed manually once per release
B. Automated and run continuously in CI/CD
C. Skipped to save time
D. Performed only by an independent test team

---

### 🔷 Q22

Which statement BEST describes the whole-team approach to testing?

A. Only testers are responsible for quality
B. Everyone contributes to quality, with testers providing specialized skills
C. Developers should not be involved in testing
D. Testers should work in a separate department

---

### 🔷 Q23

Session-Based Test Management (SBTM) for exploratory testing includes:

A. Detailed test scripts written before execution
B. Test charters, time-boxed sessions, and session reports
C. Automated execution of predefined test scripts
D. Comprehensive test plans for each session

---

### 🔷 Q24

BDD is most closely related to which other practice?

A. Load Testing
B. ATDD and Specification by Example
C. Static Analysis
D. Manual Regression Testing

---

### 🔷 Q25

A team is using Scrum. Unit testing, integration testing, and acceptance testing happen:

A. In sequential phases after all development is complete
B. Continuously throughout the sprint
C. Only during the final hardening sprint
D. Once per year during the release

---

### 🔷 Q26

Which tool is MOST appropriate for writing acceptance tests in Given-When-Then format?

A. Jenkins
B. Cucumber
C. JUnit
D. JMeter

---

### 🔷 Q27

An agile tester pairs with a developer to test a feature as it is being built. This demonstrates:

A. The elimination of independent testing
B. Collaboration between tester and developer
C. The tester having no work to do alone
D. A violation of role separation

---

### 🔷 Q28

Which is NOT a value of the Agile Manifesto?

A. Individuals and interactions over processes and tools
B. Working software over comprehensive documentation
C. Following a plan over responding to change
D. Customer collaboration over contract negotiation

---

### 🔷 Q29

In agile, the tester's role during sprint planning includes:

A. Waiting for the sprint to start before engaging
B. Reviewing user stories for testability and estimating testing effort
C. Writing detailed test plans for the entire project
D. Testing the previous sprint's code while planning happens

---

### 🔷 Q30

A system guarantees 99.9% uptime. This requirement addresses which quality characteristic?

A. Efficiency
B. Reliability
C. Security
D. Maintainability

---

### 🔷 Q31

Which test type is MOST appropriate for finding the maximum load a system can handle before failing?

A. Load Testing
B. Stress Testing
C. Endurance Testing
D. Volume Testing

---

### 🔷 Q32

In a keyword-driven framework, test cases are written as:

A. Program code in a programming language
B. A table of keywords, objects, and values
C. Recorded user interactions
D. SQL queries

---

### 🔷 Q33

Which review type is MOST formal and typically uses checklists and trained participants?

A. Informal Review
B. Walkthrough
C. Technical Review
D. Inspection

---

### 🔷 Q34

A system continues to process transactions when one server fails because other servers take over. This demonstrates:

A. Recoverability
B. Fault Tolerance
C. Maturity
D. Efficiency

---

### 🔷 Q35

Which of the following is a PRIMARY risk in test automation implementation?

A. Automated tests run too slowly
B. Underestimating the maintenance effort for automated scripts
C. Automated tests eliminate the need for testers
D. Test automation guarantees zero defects in production

---

### 🔷 Q36

A field accepts ages from 18 to 65. Using 2-value BVA, which values should be tested?

A. 18, 65
B. 17, 18, 65, 66
C. 0, 18, 65, 100
D. 18, 40, 65

---

### 🔷 Q37

In a decision table with 3 binary conditions, how many rules are needed for full coverage?

A. 6
B. 8
C. 9
D. 12

---

### 🔷 Q38

A system has two fields: X (1-10) and Y (1-10). Using single-fault 2-value BVA, how many test cases are needed?

A. 4
B. 8
C. 12
D. 16

---

### 🔷 Q39

Which of the following BEST describes exploratory testing?

A. Testing without any planning or structure
B. Simultaneous learning, test design, and test execution guided by test charters
C. Automated execution of predefined test scripts
D. Random input generation to find crashes

---

### 🔷 Q40

ROI for test automation is typically:

A. Positive in the first month
B. Negative in year 1 due to high initial development costs
C. Always 100% or higher
D. Not measurable for software testing

---

## 📝 End of Mock Exam 1
