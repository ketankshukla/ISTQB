...
A. Deploying LLM APIs to all testers immediately
B. Evaluating current testing processes, data availability, and organizational readiness
C. Eliminating all manual testing processes
D. Fine-tuning a custom model for the organization
...
A. Because GenAI tools are always technically defective
B. Because testers may perceive GenAI as a threat to job security and resist adoption
C. Because change management is only required for regulatory compliance
D. Because GenAI never requires any process changes
...
A. Mandate immediate use without training or explanation
B. Involve them in pilot design, prompt creation, and evaluation criteria
C. Threaten job loss for non-adoption
D. Replace all skeptical testers with new hires
...
A. Allowing each tester to use their own preferred prompts without standards
B. Developing organization-wide prompt templates, governance policies, and quality gates
C. Removing all human review to speed up the process
D. Switching to a different LLM provider every month
...
A. Announce that manual test design is now banned
B. Provide training on prompt engineering, output evaluation, and clear career path evolution
C. Keep the GenAI tools secret from the test team
D. Outsource all testing to a third-party AI vendor
...
A. Measuring outcomes and collecting metrics
B. Skipping human validation of GenAI outputs
C. Selecting a well-defined scope for the experiment
D. Holding regular retrospectives to refine the approach
...
A. The need to reduce printer toner costs
B. Workflow disruption and the need for new skills such as prompt engineering
C. The color scheme of the testing tools
D. The speed of office internet connections
...
A. Evaluating the organization's current testing capabilities
B. Considering domain specialization and advanced analytics
C. Running the first controlled pilot with a single test module
...
A. Allowing each team to use any public LLM without approval
B. Establishing clear accountability for AI-generated artifact quality and data sharing policies
C. Eliminating all documentation because GenAI generates everything automatically
D. Preventing testers from providing feedback on GenAI outputs
...
A. To ensure every testing task uses GenAI from day one
B. To identify high-value, low-risk pilots that demonstrate quick wins
C. To eliminate all non-GenAI testing tools immediately
D. To avoid talking to stakeholders about AI adoption
...
A. Prove the value through visible pilots and emphasize that GenAI augments, not replaces, tester expertise
B. Force them to use GenAI by making it mandatory with no exceptions
C. Agree that GenAI cannot help and abandon the initiative
D. Transfer the reluctant testers to non-testing roles
