# 📈 CTAL-TTA Progress Tracker

## ⭐ How to Use This Tracker

Check off each item as you complete it. Target: complete all items before your exam date.

---

## ⚠️ Chapter 1 — Risk-Based Testing

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## 🎯 Chapter 2 — White-Box Test Techniques

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## 🔍 Chapter 3 — Static Analysis

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## 🔬 Chapter 4 — Dynamic Analysis

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## 🛠️ Chapter 5 — Test Tools and Automation

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## ⭐ Chapter 6 — Non-Functional Testing

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## ⭐ Case Studies

- [ ] Read `case_study_1_risk_based_white_box.md`
- [ ] Read `case_study_2_static_dynamic_analysis.md`
- [ ] Read `case_study_3_non_functional_testing.md`

## 📝 Mock Exams

- [ ] Attempt `mock_exam_1.md` under timed conditions (120 min)
- [ ] Score: ___ / 45
- [ ] Review all wrong answers

- [ ] Attempt `mock_exam_2.md` under timed conditions (120 min)
- [ ] Score: ___ / 45
- [ ] Review all wrong answers

- [ ] Attempt `mock_exam_3.md` under timed conditions (120 min)
- [ ] Score: ___ / 45
- [ ] Review all wrong answers

## ⭐ Final Preparation

- [ ] Re-read `EXAM_STRATEGY.md`
- [ ] Re-read `GLOSSARY.md`
- [ ] Re-read all `exam_traps.md` files
- [ ] Schedule exam appointment
- [ ] Confirm exam location and required ID + CTFL certificate

---

## ⭐ Score Targets

| Activity | Target | Notes |
|----------|--------|-------|
| Chapter practice questions | 75%+ (15/20) | Review any chapter scored below 75% |
| Mock exam 1 | 65%+ (29/45) | Identify weak chapters |
| Mock exam 2 | 70%+ (32/45) | Improvement over mock 1 |
| Mock exam 3 | 75%+ (34/45) | Exam-ready benchmark |
