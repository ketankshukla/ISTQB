# ♟️ CTFL-GT Exam Strategy

## 🎓 Exam Format

- **40 multiple-choice questions** in 60 minutes
- **65% pass mark** (26 correct answers)
- **Closed book** — no notes or materials allowed
- Non-native English speakers may request 25% extra time (75 minutes total)

---

## 🔢 Chapter Weighting (Estimated)

| Chapter | Topic | Estimated Weight | Questions |
|---------|-------|-----------------|-----------|
| 1 | Gambling Industry Fundamentals | 20% | ~8 |
| 2 | Game Mathematics and RNG Testing | 25% | ~10 |
| 3 | Compliance, Regulations, and Player Protection | 25% | ~10 |
| 4 | Gambling Testing Types and Tools | 15% | ~6 |
| — | Cross-topic / integrated | 15% | ~6 |

**Chapters 2 and 3 carry the most weight** — master game math, RTP, RNG, and compliance.

---

## ❓ K-Level Question Patterns

| K-Level | What It Tests | Question Style | Example |
|---------|--------------|----------------|---------|
| **K1 (Remember)** | Recall definitions, terms, regulatory bodies | "Which of the following is..." | "Which body issues gambling licenses in the UK?" |
| **K2 (Understand)** | Explain concepts, compare approaches | "Which statement is TRUE..." | "Which statement about RTP is TRUE?" |
| **K3 (Apply)** | Calculate, analyze scenarios, apply techniques | Scenario with numbers | "Calculate the RTP given these payouts" |

**Pattern on this exam:**
- K1: ~10 questions (gambling terms, regulators, game types)
- K2: ~20 questions (concepts, compliance, player protection)
- K3: ~10 questions (RTP calculation, probability, game math)

---

## ⭐ Time Budgeting

| Passage | Time | Action |
|---------|------|--------|
| Questions 1-10 | ~12 min | K1 recall questions — answer quickly |
| Questions 11-25 | ~22 min | K2 concept questions — read carefully |
| Questions 26-35 | ~15 min | K3 calculation questions — work methodically |
| Questions 36-40 | ~8 min | Integrated/cross-topic questions |
| Review | ~3 min | Check flagged questions |

**Pace target:** 90 seconds per question on average.

---

## 🎓 Chapter-Specific Tactics

### 🏗️ Chapter 1 — Gambling Industry Fundamentals

- **K1 trap:** Know the major gambling domains (casino, sports betting, lottery, bingo)
- **K2 trap:** Understand the difference between game types (slots, table games, live dealer, etc.)
- **Tactic:** Memorize key terms (RNG, RTP, house edge, volatility) and which game types they apply to

### 🎮 Chapter 2 — Game Mathematics and RNG Testing

- **K3 trap:** Be ready to calculate RTP from a paytable, or probability from reel strips
- **K2 trap:** Understand what makes an RNG "suitable" (statistical randomness, unpredictability, uniformity)
- **Tactic:** Practice the worked examples in the lessons until you can do them without looking

### 🔷 Chapter 3 — Compliance, Regulations, and Player Protection

- **K1 trap:** Memorize major regulatory bodies (UKGC, MGA, GLI, etc.) and their jurisdictions
- **K2 trap:** Know the difference between player protection measures (self-exclusion, deposit limits, reality checks)
- **Tactic:** Focus on the "why" behind regulations — exam questions often ask for the rationale

### 🧪 Chapter 4 — Gambling Testing Types and Tools

- **K2 trap:** Know which test types apply to which gambling products
- **Tactic:** Think about functional vs. non-functional testing in a gambling context (RNG testing, odds verification, performance under jackpot)

---

## ⭐ Common Distractor Patterns

1. **Absolute statements** — Words like "always," "never," "eliminates," "guarantees" are usually wrong in gambling testing (regulations vary by jurisdiction)
2. **Wrong regulator for jurisdiction** — UKGC for Malta, MGA for UK, etc. Know who governs where
3. **Confusing RTP with hit frequency** — High RTP does not mean frequent wins
4. **Confusing RNG testing with game logic testing** — RNG tests randomness; game logic tests rules and payouts
5. **Player protection vs. fraud prevention** — Self-exclusion protects the player; fraud detection protects the operator. They are different.

---

## 📋 Day-Before Checklist

- [ ] Review all `exam_traps.md` files (30 min)
- [ ] Skim `GLOSSARY.md` for forgotten terms (15 min)
- [ ] Re-read `EXAM_STRATEGY.md` (10 min)
- [ ] No new content — only review what you already know
- [ ] Get adequate sleep
- [ ] Arrive early with required ID

---

## 🎓 During the Exam

1. **Flag uncertain questions** — mark them and come back later
2. **Eliminate clearly wrong answers first** — often 1-2 distractors are obviously incorrect
3. **For calculation questions:** — Write out the formula, don't try to do it mentally
4. **Read the full question** — some questions change meaning in the second half
5. **Trust your first instinct** — unless you find a clear error, your first answer is usually correct
