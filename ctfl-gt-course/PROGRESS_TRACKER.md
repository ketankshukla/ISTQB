# 📈 CTFL-GT Progress Tracker

## ⭐ How to Use This Tracker

Check off each item as you complete it. Target: complete all items before your exam date.

---

## 🏗️ Chapter 1 — Gambling Industry Fundamentals

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## 🎮 Chapter 2 — Game Mathematics and RNG Testing

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## ⭐ Chapter 3 — Compliance, Regulations, and Player Protection

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## 🧪 Chapter 4 — Gambling Testing Types and Tools

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 20
- [ ] Review all wrong answers

## ⭐ Case Studies

- [ ] Read `case_study_1_slot_game_testing.md`
- [ ] Read `case_study_2_sports_betting_platform.md`
- [ ] Read `case_study_3_regulatory_compliance_audit.md`

## 📝 Mock Exams

- [ ] Attempt `mock_exam_1.md` under timed conditions
- [ ] Score: ___ / 40
- [ ] Review all wrong answers

- [ ] Attempt `mock_exam_2.md` under timed conditions
- [ ] Score: ___ / 40
- [ ] Review all wrong answers

- [ ] Attempt `mock_exam_3.md` under timed conditions
- [ ] Score: ___ / 40
- [ ] Review all wrong answers

## ⭐ Final Preparation

- [ ] Re-read `EXAM_STRATEGY.md`
- [ ] Re-read `GLOSSARY.md`
- [ ] Re-read all `exam_traps.md` files
- [ ] Schedule exam appointment
- [ ] Confirm exam location and required ID

---

## ⭐ Score Targets

| Activity | Target | Notes |
|----------|--------|-------|
| Chapter practice questions | 80%+ (16/20) | Review any chapter scored below 80% |
| Mock exam 1 | 75%+ (30/40) | Identify weak chapters |
| Mock exam 2 | 80%+ (32/40) | Improvement over mock 1 |
| Mock exam 3 | 85%+ (34/40) | Exam-ready benchmark |
