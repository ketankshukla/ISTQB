...
D. Agile cross-functional teams
...
A. Test execution is 50% complete
B. Requirements coverage is only 60%
C. 10 defects have been found
D. 5 testers are assigned to the project
...
B. Prioritize testing based on the level of risk
C. Reduce the total number of test cases
D. Guarantee zero defects in production
...
A. Automation can completely replace manual testing
B. Automation is most valuable for repetitive, stable tests
C. Automation should be applied to all tests regardless of frequency
D. Automation requires no maintenance effort
...
A. Test plan guides test strategy, which guides test policy
B. Test policy guides test strategy, which guides test plan
C. They are independent documents with no relationship
D. Test strategy and test policy are the same thing
...
C. A usability test requiring subjective evaluation
...
A. A risk that the product will have defects
B. A risk that the project will fail to meet its objectives
C. A risk that cannot be tested
D. A risk specific to test execution
...
A. Key tester resigns during the project
B. The system crashes under peak load
C. Test environment is delayed
D. Budget is cut by 20%
...
B. Only the test manager (functional manager)
C. Both the project manager and the functional manager
D. No one; they are self-directed
...
A. The project is identical to a previous project
B. Experts have diverse perspectives and the project is novel
C. Historical data is very accurate
D. The schedule is already fixed
...
B. Uses defined rules, checklists, and collects metrics
...
B. Validate that the tool solves the identified problem in the real environment
C. Train all users on the tool
D. Obtain a discount from the vendor
...
A. Recognition for finding critical defects
B. Being blamed for project delays
C. Access to modern testing tools
D. Clear and achievable goals
...
B. When testing should be temporarily halted until a condition is resolved
...
B. Integration with existing development and project management tools
C. The number of animated icons
D. Whether the tool has a mobile app
...
A. A test strategy is for the organization; a test approach is for a specific project
B. A test strategy is high-level; a test approach is the implementation of the strategy
C. They are identical terms
D. A test approach is only used in agile projects
