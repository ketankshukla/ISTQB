# CTAL-TM Progress Tracker

## How to Use This Tracker

Check off each item as you complete it. Target: complete all items before your exam date.

---

## Chapter 1 — Testing Process

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 25
- [ ] Review all wrong answers

## Chapter 2 — Test Management

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 25
- [ ] Review all wrong answers

## Chapter 3 — Reviews

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 25
- [ ] Review all wrong answers

## Chapter 4 — Test Tools and Automation

- [ ] Read `README.md`
- [ ] Read `lessons.md`
- [ ] Read `exam_traps.md`
- [ ] Attempt `practice_questions.md`
- [ ] Score: ___ / 25
- [ ] Review all wrong answers

## Case Studies

- [ ] Read `case_study_1_test_strategy_planning.md`
- [ ] Read `case_study_2_risk_based_testing.md`
- [ ] Read `case_study_3_test_team_transformation.md`

## Mock Exams

- [ ] Attempt `mock_exam_1.md` under timed conditions (180 min)
- [ ] Score: ___ / 65
- [ ] Review all wrong answers

- [ ] Attempt `mock_exam_2.md` under timed conditions (180 min)
- [ ] Score: ___ / 65
- [ ] Review all wrong answers

- [ ] Attempt `mock_exam_3.md` under timed conditions (180 min)
- [ ] Score: ___ / 65
- [ ] Review all wrong answers

## Final Preparation

- [ ] Re-read `EXAM_STRATEGY.md`
- [ ] Re-read `GLOSSARY.md`
- [ ] Re-read all `exam_traps.md` files
- [ ] Schedule exam appointment
- [ ] Confirm exam location and required ID + CTFL certificate

---

## Score Targets

| Activity | Target | Notes |
|----------|--------|-------|
| Chapter practice questions | 75%+ (19/25) | Review any chapter scored below 75% |
| Mock exam 1 | 65%+ (42/65) | Identify weak chapters |
| Mock exam 2 | 70%+ (46/65) | Improvement over mock 1 |
| Mock exam 3 | 75%+ (49/65) | Exam-ready benchmark |
